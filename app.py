...
def get_profile_info(profile_name):
    ...
    return (
        ...
        profile.get("default_num_step", 32),
...
def generate_single_audio(text, num_step, guidance_scale, speed):
    """Generate single audio with OmniVoice"""
    if model is None:
...
def process_project_chapters(start_chapter, end_chapter, regenerate, enable_srt, min_words, max_chars, progress=gr.Progress()):
    """Process project chapters - with configurable SRT generation using OmniVoice"""
    if model is None:
...
with gr.Blocks(title="OmniVoice TTS", theme=gr.themes.Soft()) as app:
    gr.Markdown("# 🎙️ OmniVoice Text-to-Speech with Project Management")
    ...
    with gr.Tabs():
        ...
        with gr.Tab("👤 Profile Management"):
            ...
            with gr.Row():
                ...
                with gr.Column():
                    prompt_text_input = gr.Textbox(label="Reference Text (optional, Whisper ASR if empty)", value=get_current_config().get("prompt_text", "") or "", lines=3)
                    output_dir_input = gr.Textbox(label="Output Directory", value=get_current_config().get("output_dir", ""))
            
            with gr.Row():
                num_step_input = gr.Slider(4, 32, get_current_config().get("default_num_step", 32), step=1, label="Num Steps (high=better quality, low=faster)")
                guidance_scale_input = gr.Slider(0.0, 5.0, get_current_config().get("default_guidance_scale", 2.0), step=0.1, label="Guidance Scale")
                speed_input = gr.Slider(0.5, 2.0, get_current_config().get("default_speed", 1.0), step=0.1, label="Speed Factor")
        ...
        with gr.Tab("🎵 Generate Single Audio"):
            gr.Markdown("### Generate audio from text using OmniVoice")
            gr.Markdown(f"*Using profile: **{current_profile}***")
            
            text_input = gr.Textbox(label="Text to synthesize", lines=5, placeholder="Enter text here...")
            
            with gr.Row():
                single_num_step = gr.Slider(4, 32, get_current_config().get("default_num_step", 32), step=1, label="Num Steps")
                single_guidance = gr.Slider(0.0, 5.0, get_current_config().get("default_guidance_scale", 2.0), step=0.1, label="Guidance Scale")
            ...
            generate_btn.click(
                ...
                inputs=[text_input, single_num_step, single_guidance, single_speed],
        ...
        with gr.Tab("❓ Help"):
            ...
            - Configure OmniVoice parameters (device_map, dtype, reference audio)
            - Go to **Model Management** → Load Model (downloads k2-fsa/OmniVoice from HuggingFace)
            ...
            - Check **Generate SRT subtitles** to create subtitle files
            
            ### 🎤 OmniVoice Parameters
            - **Device Map**: "auto" (auto-select), "cpu" (CPU only), or GPU ID
            - **Data Type**: "float16" (faster, less VRAM) or "float32" (more accurate)
            - **Reference Audio**: Voice sample to clone (~2-5 seconds recommended)
            - **Reference Text**: Transcription of reference audio (Whisper ASR used if empty)
            - **Num Steps**: 32 for best quality, 16+ for faster generation
            - **Guidance Scale**: 2.0 default (higher = closer to reference voice)
            - **Speed**: 1.0 = normal speed
            
            ### 💡 Key Features
            - Automatic voice cloning from reference audio
            - Whisper ASR auto-transcription support
            - SRT subtitle generation with fine-grained timing
            - Batch processing for large audiobooks
